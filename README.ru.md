...
Кроссплатформенное настольное приложение для просмотра истории диалогов Claude Code и Cursor IDE, хранящейся в `~/.claude` и папках данных Cursor.
...
Claude Code и Cursor IDE сохраняют историю диалогов в файлах JSONL в своих папках данных (`~/.claude/projects/` для Claude Code и папка AppData для Cursor). Эти файлы трудночитаемы и неудобны для поиска. Данное приложение предоставляет единый интерфейс для просмотра диалогов из обоих инструментов, статистики использования и поиска старых обсуждений.
...
- Усиление политики безопасности контента (CSP) для защиты от XSS-атак
- Исправления кроссплатформенной обработки путей для совместимости с Windows
- Защита от деления на ноль в аналитических вычислениях
- Исправление отслеживания прогресса загрузки в автообновлении
- Интеграция CodeRabbit AI для ревью кода
...
**🎨 Улучшенный UI/UX**:
- Улучшенный светлый/темный режим и стилизация пузырьков сообщений
- Очистка выбора кнопкой X и клавишей ESC
- Улучшенное отображение заголовков сессий
- Унифицированная архитектура состояния представлений

**🔧 Опыт разработчика**:
- Полный набор E2E-тестов с использованием Playwright
- Автоматизированный процесс релизов через GitHub Actions
- Улучшенная документация (см. CLAUDE.md)
...
2. Оно автоматически просканирует `~/.claude` для поиска данных диалогов
...
- Доступ только для чтения - невозможно редактировать или удалять диалоги из приложения
- Пока нет функции экспорта (планируется в будущих релизах)

## Конфиденциальность данных

Полностью работает локально. Никакие данные не отправляются на серверы. Приложение читает только ваш каталог `~/.claude`.
