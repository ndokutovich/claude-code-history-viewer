...
pub type WatcherMap = Arc<Mutex<Option<Debouncer<RecommendedWatcher>>>>;
...
fn handle_file_event(app_handle: &AppHandle, event: &DebouncedEvent) {
    ...
    let event_type = match event.kind {
        DebouncedEventKind::Any | DebouncedEventKind::AnyContinuous => "session-file-changed",
        _ => "session-file-changed",
    };
...
mod tests {
    ...
    fn test_extract_paths_nested() {
        let path =
            PathBuf::from("/Users/test/.claude/projects/my-project/subfolder/session.jsonl");
        let result = extract_paths(&path);
