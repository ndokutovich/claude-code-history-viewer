...
pub fn claude_message_to_universal(
    ...
    // Preserve project_name if present (used in search results)
    if let Some(ref proj_name) = msg.project_name {
        provider_metadata.insert("project_path".to_string(), json!(proj_name));
    }
