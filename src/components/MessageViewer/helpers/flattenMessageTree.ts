...
import type { TaskOperation, TaskInfo } from "./taskOperationHelpers";
 ...
 */
function mergeCommandOutputMessages(messages: UIMessage[]): UIMessage[] {
  // Build uuid lookup
  const uuidMap = new Map<string, UIMessage>();
  ...
  for (const msg of messages) {
    ...
    const parentContent = extractUIMessageContent(parent);
    if (!parentContent || typeof parentContent !== "string") continue;
    if (!/<command-name>/.test(parentContent)) continue;

    // Merge: combine content strings
    const mergedContent = parentContent + "\n" + content;
    const updatedParent: UIMessage = { ...parent, content: mergedContent };
...
export function flattenMessageTree({
  ...
  const childrenMap = new Map<string | null, UIMessage[]>();
  ...
  const sortByTimestamp = (a: UIMessage, b: UIMessage): number => {
  ...
  const orderedMessages: UIMessage[] = [];
  const visited = new Set<string>();

  function traverse(message: UIMessage, skipDueToHiddenParent = false): void {
...
function createFlattenedMessage(
  ...
  return {
    ...
    taskOperationGroup: isTaskOperationGroupLeader ? taskOpGroupInfo!.operations as TaskOperation[] : undefined,
    taskRegistry: isTaskOperationGroupLeader ? taskOpGroupInfo!.taskRegistry as Map<string, TaskInfo> : undefined,
  };
}
