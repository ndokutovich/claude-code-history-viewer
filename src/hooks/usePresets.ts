...
import type { PresetData, PresetInput, UserSettings } from "../types/presets";
import { settingsToJson, jsonToSettings } from "../types/presets";
...
export interface UsePresetsResult {
  ...
  applyPreset: (id: string) => Promise<UserSettings>;
...
export const usePresets = (): UsePresetsResult => {
  ...
  // Apply a preset (load settings from preset)
  const applyPreset = useCallback(
    async (id: string): Promise<UserSettings> => {
      setIsLoading(true);
      setError(null);
  ...
  // Create a preset from provided settings
  const createPresetFromSettings = useCallback(
    async (name: string, settings: UserSettings, description?: string): Promise<PresetData> => {
